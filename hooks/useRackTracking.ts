...
export function useRackTracking() {
  const [rackTracking, setRackTracking] = useState<Record<string, RackTrackingData>>({})
  ...
  const trackOverheatEvent = useCallback((rackId: string, temperature: number) => {
  ...
  const recordFanBoost = useCallback((rackId: string, percentBoost: number, initialTemp: number) => {
    setRackTracking((prev) => {
      const rackData = prev[rackId]
      if (!rackData) return prev

      const fanBoostAction: FanBoostAction = {
        timestamp: Date.now(),
        percentBoost,
        initialTemp,
      }
      ...
      return {
        ...
        [rackId]: {
          ...rackData,
          fanBoostActions: [...rackData.fanBoostActions, fanBoostAction],
          lastFanBoost: fanBoostAction,
        },
      }
    })
  ...
  const recordTempRecovery = useCallback((rackId: string, currentTemp: number) => {
    setRackTracking((prev) => {
      const rackData = prev[rackId]
      ...
      if (timeSinceBoost > 4000 && currentTemp < rackData.lastFanBoost.initialTemp) {
        // 4 seconds
        ...
        return {
          ...
          [rackId]: {
            ...rackData,
            tempRecoveryRecords: [...rackData.tempRecoveryRecords, recoveryRecord],
  ...
  const getMaintenancePrediction = useCallback(
    (rackId: string) => {
      ...
      if (!rackData) return { days: 28, status: "good", message: "Normal maintenance schedule" }

      const now = Date.now()
      const last72Hours = now - 72 * 60 * 60 * 1000

      const recentOverheats = rackData.overheatEvents.filter((event) => event.timestamp > last72Hours)
  ...
  const getCoolingEfficiency = useCallback(
    (rackId: string) => {
      ...
      const totalTempDrop = recentRecords.reduce((sum, record) => sum + (record.startTemp - record.endTemp), 0)
      const totalBoosts = rackData.fanBoostActions.slice(-10).reduce((sum, action) => sum + action.percentBoost, 0)

      if (totalBoosts === 0) return 85

      // Calculate efficiency as temperature drop per unit of fan boost
      const efficiency = Math.min(100, Math.max(0, (totalTempDrop / totalBoosts) * 100))
  ...
  const getOverheatCount = useCallback(
    (rackId: string, hours = 72) => {
      ...
      return rackData.overheatEvents.filter((event) => event.timestamp > cutoffTime).length
    },
    [rackTracking],
  )
  ...
  const getRackStats = useCallback(
    (rackId: string) => {
      ...
      return {
        ...
        recentOverheats: getOverheatCount(rackId, 72),
        ...
        maintenancePrediction: getMaintenancePrediction(rackId),
        coolingEfficiency: getCoolingEfficiency(rackId),
      }
    },
    [rackTracking, getOverheatCount, getMaintenancePrediction, getCoolingEfficiency],
  )
